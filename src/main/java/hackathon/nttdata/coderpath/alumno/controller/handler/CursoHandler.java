package hackathon.nttdata.coderpath.alumno.controller.handler;
...
import hackathon.nttdata.coderpath.alumno.controller.validation.ObjectValidator;
...
public class CursoHandler {
	...
	public Mono<ServerResponse> listar(ServerRequest request) {
		...
		return ServerResponse.ok().contentType(APPLICATION_JSON_UTF8).body(service.findCursos(), Cursos.class);
	}
	...
	public Mono<ServerResponse> save(ServerRequest request) {
		Mono<Cursos> dtoMono = request.bodyToMono(Cursos.class).doOnNext(objectValidator::validate);
	...
	public Mono<ServerResponse> delete(ServerRequest request) {
		String id = request.pathVariable("id");
		return ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(service.deleteCurso(id), Cursos.class);
	}
